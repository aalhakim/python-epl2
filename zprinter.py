...
class ZebraPrinter(zebra):
    ...
    def _find_printer_unix(self):
        ...
        if found_printer is not None:
            ...
            print("found '{}' but it is not setup at localhost:631/admin".format(found_device))
        else:
            print("failed.")
...
class EplCommand(object):
    ...
    def set_size_properties(self, label_width_mm, label_length_mm, columns,
                            label_gap_mm, force_configure=True):
    ...
    def _set_label_length(self, label_length_mm):
        ...
        assert(16 <= self.label_gap_dot <= 240), "Invalid setting (gap_length of {}): 16 <= gap_length <= 240".format(self.label_gap_dot)
    ...
    def _set_label_width(self, label_width_mm, columns):
        ...
        reel_width_dot = int((columns * self.label_width_dot) + ((columns - 1) * self.label_gap_dot))
    ...
    def encode_barcode_c128(self, string, start_x=0, start_y=0, height_dot=10):
        ...
        """
        start_x = int(start_x) + ((self.label_width_dot + self.label_gap_dot) * self.active_column) + self.x_offset_dot
        if start_x < 0:
            start_x = 0
        ...
        barcode = ('B{},{},0,1,{},{},{},N,"{}"\n'
                   .format(start_x, start_y, narrow_bar, wide_bar, height_dot, string))
    ...
    def encode_qr_code(self, data, start_x=0, start_y=0):
        ...
        barcode = ('b{},{},{},Q,m2,s{},e{},iA,"{}"\n'
                   .format(byte_size, start_x, start_y, scale_factor, error_correction, data))
        self.command += barcode
    ...
    def encode_aztec_code(self, data, start_x=0, start_y=0):
        ...
        # "{}": Data for encoding.
        barcode = ('b{},{},A,d{},e{},"{}"\n'
                   .format(start_x, start_y, scale_factor, error_correction, data))
        self.command += barcode

    def encode_binary_image(self, byte_string, width_dot, length_dot, start_x=0, start_y=0):
        """
        ...
        image = ('GW{},{},{},{}\n{}\n'
                 .format(start_x, start_y, width_bytes, length_dot, byte_string))
        self.command += image

    def encode_text(self, string, start_x, start_y, font_size=3, rotate=0, expand_x=1, expand_y=1, invert=False):
        """
        ...
        text = ('A{},{},{},{},{},{},{},"{}"\n'
                .format(start_x, start_y, rotate_id, font_size, expand_x, expand_y, invert, string))
        self.command += text
    ...
    def encode_box(self, width_dot, length_dot, stroke_dot, start_x, start_y):
        ...
        box = (
            ...
            'LO{6},{1},{3},{4}\n'  # Right vertical line
            .format(start_x, start_y,                   # 0, 1
                    width_dot, stroke_dot, length_dot,  # 2, 3, 4
                    end_y, end_x)                       # 5, 6
            )
        self.command += box
...
class Printer(object):
    ...
    def __init__(self, printer, label_width_mm, label_length_mm, label_length_adjust_mm,
                 columns, gap_size_mm, x_offset_mm=0.0, y_offset_mm=0.0):
        """
        Args:
        ...
        self.columns = columns
        self.commander = EplCommand(printer, label_width_mm, label_length_mm + label_length_adjust_mm,
                                    columns, gap_size_mm, x_offset_mm, y_offset_mm)
    ...
    def print_calibrator(self):
        ...
        objBox = Box(self.label.width_mm - 1.0,
                     self.label.length_mm - 1.0,
                     stroke_mm=0.125)
        ...
        for n in range(1, self.columns + 1):
            self.commander.encode_box(width_dot=objBox.width_dot,
                                      length_dot=objBox.length_dot,
                                      stroke_dot=objBox.stroke_dot,
                                      start_x=xpos, start_y=ypos)

            self.commander.encode_text(objText_text1.text, start_x=text1_xpos,
                                     start_y=text1_ypos, font_size=1)

            self.commander.encode_text(objText_text2.text, start_x=text2_xpos,
                                     start_y=text2_ypos, font_size=1)

            self.commander.encode_text(objText_text3.text, start_x=text3_xpos,
                                     start_y=text3_ypos, font_size=1)

            self.commander.encode_text(objText_text4.text, start_x=text4_xpos,
                                     start_y=text4_ypos, font_size=1)

            self.commander.encode_text(objText_text5.text, start_x=text5_xpos,
                                     start_y=text5_ypos, font_size=1)

            self.commander.encode_text(objText_text6.text, start_x=text6_xpos,
                                     start_y=text6_ypos, font_size=1)
    ...
    def print_barcode(self, text, quantity=1, bc_height_mm=None, invert=False, override_exit=False):
        ...
        objBarcode = Barcode(text, height_mm=bc_height_mm, max_width_mm=self.label.width_mm)
        ...
        text_ypos    = objBarcode.height_dot + int((4 - font_size) * 0.2 * objText.height_dot)

        # Submit the print command.
        for n in range(1, quantity + 1):
            self.commander.encode_barcode_c128(objBarcode.text, start_x=bc_xpos, start_y=bc_ypos,
                                               height_dot=objBarcode.height_dot)

            self.commander.encode_text(objText.text, start_x=text_xpos, start_y=text_ypos,
                                       font_size=objText.font_size, invert=invert)
    ...
    def print_binary_image(self, binary_data, quantity=1, centre=True, override_exit=False):
        """ Print a barcode with some text below it.
        ...
        for n in range(1, quantity + 1):
            self.commander.encode_binary_image(byte_string,
    ...
    def print_box(self, width_mm, length_mm, stroke_mm, x_align="C", y_align="M", quantity=1,
                  override_exit=False):
    ...
    def print_title(self, text_list, override_exit=False):
        ...
        for i, string in enumerate(text_list, start=1):
            ...
            self.commander.encode_text(objText.text,
                                       start_x=xpos, start_y=ypos,
    ...
    def print_qr_with_text(self, text, quantity=1, scale=3, override_exit=False):
        ...
        erp_ypos = (qr_ypos + image.length_dot) + int((4 - erp_font_size) * 0.2 * objText_erp.height_dot)
        uid_ypos =                     erp_ypos + int((4 - uid_font_size) * 0.2 * objText_uid.height_dot) + 15
        ...
        for n in range(1, quantity + 1):
            ...
            self.commander.encode_text(objText_erp.text,
                                       start_x=erp_xpos, start_y=erp_ypos,
                                       font_size=objText_erp.font_size, invert=False)

            self.commander.encode_text(objText_uid.text,
                                       start_x=uid_xpos, start_y=uid_ypos,
                                       font_size=objText_uid.font_size, invert=False)
...
if __name__ == '__main__':
    ...
    zebraPrinter = Printer(zp, LABEL_WIDTH_MM, LABEL_LENGTH_MM, LENGTH_ADJUST_MM,
