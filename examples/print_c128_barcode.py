...
if __name__ == '__main__':
    ...
    for n in range(quantity):
        ...
        if last_label is True and n % SCROLL_COLUMNS != max_column:
            zebraPrinter.print_barcode(serial_number, quantity=1, bc_height_mm=height, invert=True, override_exit=False)
        else:
            zebraPrinter.print_barcode(serial_number, quantity=1, bc_height_mm=height, invert=False, override_exit=True)
