...
if __name__ == '__main__':
    ...
    for n in range(quantity):
        ...
        if last_label is True and n % SCROLL_COLUMNS != max_column:
            zebraPrinter.print_box(width_mm=LABEL_WIDTH_MM/(n+1), length_mm=LABEL_LENGTH_MM/(n+1),
                                   stroke_mm=STROKE_WIDTH, x_align=X_ALIGN, y_align=Y_ALIGN,
                                   quantity=1, override_exit=False)

        else:
            zebraPrinter.print_box(width_mm=LABEL_WIDTH_MM/(n+1), length_mm=LABEL_LENGTH_MM/(n+1),
                                   stroke_mm=STROKE_WIDTH, x_align=X_ALIGN, y_align=Y_ALIGN,
                                   quantity=1, override_exit=True)
